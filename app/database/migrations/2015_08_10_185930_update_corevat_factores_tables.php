...
class UpdateCorevatFactoresTables extends Migration {
	...
	{
        $sql = <<<EoFn
            DROP FUNCTION IF EXISTS update_factores();
            CREATE OR REPLACE FUNCTION update_factores() RETURNS void AS $$
            DECLARE
                v_row1 RECORD;
            BEGIN
                -- FACTOR CONSERVACION
                FOR v_row1 IN SELECT idfactorconservacion, factor_conservacion, valor_factor_conservacion FROM cat_factores_conservacion ORDER BY idfactorconservacion LOOP
                    ...
                    --UPDATE aef_construcciones SET fk_conservacion = v_row1.idfactorconservacion WHERE zona = v_row1.factor_conservacion;
                    --UPDATE aef_terrenos SET fk_top = v_row1.idfactorconservacion WHERE top = v_row1.factor_conservacion;
                END LOOP;
                ...
                FOR v_row1 IN SELECT idfactorfrente, valor_factor_frente FROM cat_factores_frente ORDER BY idfactorfrente LOOP
                    UPDATE aem_homologacion SET fk_zona = v_row1.idfactorfrente WHERE frente = v_row1.valor_factor_frente;
                    UPDATE aef_terrenos SET fk_forma = v_row1.idfactorfrente WHERE frente = v_row1.valor_factor_frente;
                END LOOP;

                -- FACTOR SUPERFICIE
                FOR v_row1 IN SELECT idfactorsuperficie, minimo, maximo FROM cat_factores_superficie ORDER BY idfactorsuperficie LOOP
                    --UPDATE aem_homologacion SET fk_superficie = v_row1.idfactorzona WHERE zona = v_row1.valor_factor_zona;
                END LOOP;
...
EoFn;
        ...
        DB::connection('corevat')->getPdo()->exec("SELECT update_factores()");
        DB::connection('corevat')->getPdo()->exec("DROP FUNCTION IF EXISTS update_factores()");
	}
	...
	{
        $sql = "DROP FUNCTION IF EXISTS update_factores()";
