...
class CorevatCatEstructuras extends Migration {
	...
	public function up()
	{
		Schema::connection('corevat')->create('cat_estructuras', function(Blueprint $table) {
			$table->increments('idestructura');
			$table->string('estructura', 100);
			$table->integer('status_estructura')->default(1);
	...
	{
		DB::connection('corevat')->getPdo()->exec("DROP TABLE IF EXISTS cat_estructuras CASCADE;");
	}
