...
class CorevatCatPisos extends Migration {
	...
	public function up()
	{
		Schema::connection('corevat')->create('cat_pisos', function(Blueprint $table) {
			$table->increments('idpiso');
			$table->string('piso', 100);
			$table->integer('status_piso')->default(1);
	...
	{
		DB::connection('corevat')->getPdo()->exec("DROP TABLE IF EXISTS cat_pisos CASCADE;");
	}
