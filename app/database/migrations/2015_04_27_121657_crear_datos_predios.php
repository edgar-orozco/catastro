...
class CrearDatosPredios extends Migration {
    ...
    public function up() {
        ...
        if (count($existe) > 0) {
            ...
            DB::statement("DROP FUNCTION IF EXISTS sp_get_datos_predio(IN p_clave TEXT)");
        }
...
RETURNS TABLE (clave VARCHAR, propietario TEXT, municipio VARCHAR, direccion TEXT, impuesto NUMERIC, valor_catastral NUMERIC, anio_periodo VARCHAR, adeudo NUMERIC, sup_const FLOAT, superficie_terreno NUMERIC) 
AS 
$$
DECLARE
	v_consulta	TEXT;
BEGIN
	...
	SELECT  pf.clave  clave
				 ...
				 , sp_get_supconstruccion (substr(pf.clave, 8,15)) sup_const
