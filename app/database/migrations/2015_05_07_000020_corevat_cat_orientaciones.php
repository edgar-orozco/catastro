...
class CorevatCatOrientaciones extends Migration {
	...
	public function up()
	{
		Schema::connection('corevat')->create('cat_orientaciones', function(Blueprint $table) {
			$table->increments('idorientacion');
			$table->string('orientacion', 100);
			$table->integer('status_orientacion')->default(1);
	...
	{
		DB::connection('corevat')->getPdo()->exec("DROP TABLE IF EXISTS cat_orientaciones CASCADE;");
	}
