...
class corevat_PrintDictamenAvaluoController extends \BaseController {
	 ...
	 * @return Void
	 */
	private function avaluoEncabezado(&$pdf) {
		$pdf->SetFillColor(64, 64, 64);
		$pdf->AddPage();
		$pdf->Image(public_path() . "/css/images/corevat/crv-01.jpg", 5, 5, 139.57, 26.20);
		$user = Auth::user();
		...
		$pdf->Ln(1);
		$pdf->setX(5);
		$pdf->Cell(206, 6, utf8_decode('A V A L Ú O'), 'TLBR', 1, 'C', 0);
	...
	// DATOS DEL VALUADOR
	//*********************************************************
	private function avaluoDatosValuador(&$pdf, $rs) {
		$pdf->Ln(2);
		$nFont = 4;
		...
		//$pdf->Ln(2);
		$pdf->setX(5);
		$pdf->Cell(206, 5, 'DATOS DEL VALUADOR', 'TLBR', 1, 'C', 1);
	...
	private function avaluoInformacionGeneralInmueble(&$pdf, $nFont, $rs) {
		$pdf->Ln(2);
		$nFont = 4;
		...
		$pdf->SetFillColor(164, 164, 164);
		$pdf->setX(5);
		$pdf->Cell(206, 5, 'INFORMACION GENERAL DEL INMUEBLE', 'TLBR', 1, 'C', 1);
	...
	public function printAvaluo($id) {
		...
		$this->avaluoEncabezado($pdf);

		$this->avaluoDatosValuador($pdf, $rs);
		
		$this->avaluoInformacionGeneralInmueble($pdf, $nFont, $rs);
		...
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(40, $nFont * 3, utf8_decode("Servicios municipales: "), 'LBR', 0, 'R');
		...
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(30, $nFont, utf8_decode("Recolección basura: "), 'B', 0, 'R');
		$pdf->SetFont('Arial', '', 8);
		$pdf->Cell(3.2, $nFont, $zn->is_recoleccion_basura == 1 ? 'X' : '', 'LBR', 1, 'L');
		
		// linea 3
		$pdf->setX(45);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(30, $nFont, utf8_decode("Vigilancia privada: "), 'B', 0, 'R');
		$pdf->SetFont('Arial', '', 8);
		$pdf->Cell(3.2, $nFont, $zn->is_vigilancia_privada == 1 ? 'X' : '', 'LBR', 0, 'L');

		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(30, $nFont, utf8_decode("Internet: "), 'B', 0, 'R');
		$pdf->SetFont('Arial', '', 8);
		$pdf->Cell(3.2, $nFont, $zn->is_internet == 1 ? 'X' : '', 'LBR', 0, 'L');
		
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(30, $nFont, utf8_decode("Otros: "), 'B', 0, 'R');
		$pdf->SetFont('Arial', '', 8);
		$pdf->Cell(3.2, $nFont, $zn->is_otro_servicio == 1 ? 'X' : '', 'LBR', 0, 'L');
		$pdf->Cell(66.4, $nFont, $zn->otro_servicio_municipal == 1 ? 'X' : '', 'LBR', 1, 'L');
		
		
		// Línea 4
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(40, $nFont * 2, utf8_decode("Equipamiento urbano: "), 'LBR', 0, 'R');
		...
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(40, $nFont, utf8_decode("En un radio: "), 'LBR', 0, 'R');
		$pdf->SetFont('Arial', '', 8);
		$pdf->Cell(10, $nFont, utf8_decode($zn->cobertura), 'BR', 0, 'R');
		...
		$pdf->SetFont('Arial', '', 8);
		$pdf->Cell(66, $nFont, $zn->clasificacion_zona, 'LBR', 1, 'L');

		/*
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(40, $nFont, utf8_decode("Proximidad urbana: "), 'LBR', 0, 'R');
		$pdf->SetFont('Arial', '', 8);
		$pdf->Cell(30, $nFont, $zn->proximidad_urbana, 'LBR', 1, 'L');
		*/
		...
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->MultiCell(40, 3.60, utf8_decode("Construcciones Predominantes:"), '', 'R');
		...
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->MultiCell(40, 3.60, utf8_decode("Vías de acceso e importancia:"), '', 'R');
		...
		$pdf->SetFillColor(164, 164, 164);
		$pdf->setX(5);
		$pdf->Cell(206, 5, utf8_decode('CARACTERISTÍCAS DEL INMUEBLE (1 de 2)'), 'TLBR', 1, 'C', 1);
		...
		if ($in->croquis != "") {
			...
			if (file_exists($archivo)) {
				...
				$archivo = public_path() . '/corevat/files/' . $in->croquis;
				if (file_exists($archivo)) {
					$pdf->Image($archivo, 5, null, 89.0, 65.40);
				} else {
					$pdf->Image(public_path() . '/css/images/corevat/blank.gif', 5, null, 89.0, 65.40);
				}
				 */
			}
		} else {
		...
		if ($in->fachada != "") {
			...
			if (file_exists($archivo)) {
				...
				$archivo = public_path() . '/corevat/files/' . $in->fachada;
				if (file_exists($archivo)) {
					$pdf->Image($archivo, 122, null, 89.0, 65.40);
				} else {
		...
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 10);
		$pdf->Cell(206, 5, utf8_decode('A) MEDIDAS Y COLINDANCIAS'), 'TLBR', 1, 'L', 0);
		
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(20, 4, utf8_decode('Según: '), 'TLBR', 0, 'R', 0);
		$pdf->SetFont('Arial', '', 8);
		$pdf->Cell(186, 4, utf8_decode($in->segun), 'TLBR', 1, 'L', 0);
		...
		$pdf->setX(50);
		$pdf->SetFont('Arial', 'B', 6);
		$pdf->Cell(30, 4, utf8_decode('Orientación'), 'TLBR', 0, 'C', true);
		$pdf->Cell(30, 4, utf8_decode('U. M.'), 'TLBR', 0, 'C', true);
		$pdf->Cell(30, 4, utf8_decode('Medidas'), 'TLBR', 0, 'C', true);
		$pdf->Cell(30, 4, utf8_decode('Colindancias'), 'TLBR', 1, 'C', true);
		$i = 0;
		
		foreach ($rstMedCol as $medcol) {
			$pdf->setX(50);
			$pdf->SetFont('Arial', 'B', 6);
			$pdf->Cell(30, $nFont, utf8_decode($medcol->orientacion), 'BL', 0, 'L', 0);
			$pdf->SetFont('Arial', '', 6);
			$pdf->Cell(30, $nFont, utf8_decode($medcol->unidad_medida), 'BLR', 0, 'L', 0);
			$pdf->Cell(30, $nFont, utf8_decode($medcol->medidas), 'BL', 0, 'L', 0);
			$pdf->Cell(30, $nFont, utf8_decode($medcol->colindancia), 'BLR', 1, 'L', 0);
			$i++;
			if ($i == 7) {
				$pdf->AddPage();
				$pdf->setX(50);
		...
		*/
		...
		$pdf->SetFont('Arial', 'B', 10);
		$pdf->setX(5);
		$pdf->Cell(206, 5, utf8_decode('B) CARACTERÍSTICAS DE LA CONSTRUCCIÓN'), 'TBLR', 1, 'L', 0);
		...
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(30, $nFont, utf8_decode("Niveles de la Unidad: "), 'LB', 0, 'R');
		$pdf->SetFont('Arial', '', 8);
		...
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->MultiCell(30, 3.60, utf8_decode("Descripcion General del Inmueble:"), '', 'R');
		...
		$pdf->SetFillColor(164, 164, 164);
		$pdf->setX(5);
		$pdf->Cell(206, 5, utf8_decode('CARACTERÍSTICAS DEL INMUEBLE (2 de 2)'), 'TLBR', 1, 'C', 1);

		$pdf->SetFont('Arial', 'B', 10);
		$pdf->setX(5);
		$pdf->Cell(206, 5, utf8_decode('C) ACABADOS'), 'TBLR', 1, 'L', 0);
		// ENCABEZADO DE LA TABLA DE ACABADOS
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(40, $nFont, utf8_decode("Acabado"), 'TLB', 0, 'C', true);
		$pdf->Cell(55.3333333333333333333, 4, utf8_decode("Pisos"), 'TLB', 0, 'C', true);
		$pdf->Cell(55.3333333333333333333, 4, utf8_decode("Muros"), 'TLB', 0, 'C', true);
		$pdf->Cell(55.3333333333333333333, 4, utf8_decode("Plafones"), 'TLBR', 1, 'C', true);
		...
		foreach ($rows as $row) {
			...
			$pdf->Cell(55.3333333333333333333, 4, utf8_decode($row['piso']), 'LB', 0, 'L', false);
			$pdf->Cell(55.3333333333333333333, 4, utf8_decode($row['aplanado']), 'LB', 0, 'L', false);
			$pdf->Cell(55.3333333333333333333, 4, utf8_decode($row['plafon']), 'LRB', 1, 'L', false);
		}
		...
		*/
		...
		$pdf->SetFont('Arial', 'B', 10);
		$pdf->setX(5);
		$pdf->Cell(206, 5, utf8_decode('D) SUPERFICIES'), 'TBLR', 1, 'L', 0);
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 8);
		...
		$pdf->SetFillColor(164, 164, 164);
		$pdf->setX(5);
		$pdf->Cell(206, 5, utf8_decode('ENFOQUE DE MERCADO'), 'TLBR', 1, 'C', 1);

		//Comparables de terrenos (aem_comp_terrenos)
		$pdf->setX(5);
		$pdf->Cell(206, 5, utf8_decode('A) VENTA DE TERRENOS'), 'BLR', 1, 'L', 0);
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 7);
		$pdf->SetFillColor(208, 208, 208);
		...
		$pdf->SetFillColor(164, 164, 164);
		$pdf->setX(5);
		$pdf->Cell(206, 5, utf8_decode('ANÁLISIS FÍSICO'), 'TLBR', 1, 'C', 1);
		$pdf->setX(5);
		$pdf->Cell(206, 5, utf8_decode('A) TERRENO'), 'BLR', 1, 'L', 0);
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 8);
		...
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 10);
		$pdf->Cell(206, 5, utf8_decode('B) CONSTRUCCIONES'), 'TBLR', 1, 'L', 0);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->setX(5);
		...
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(206, 4, utf8_decode('C1)  COMUNES, INSTALACIONES ESPECIALES Y OBRAS COMPLEMENTARIAS (SOLO EN CONDOMINIOS)'), 'TBLR', 1, 'L', 0);
		...
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(206, 4, utf8_decode('C2)  PRIVATIVAS, INSTALACIONES ESPECIALES Y OBRAS COMPLEMENTARIAS'), 'TBLR', 1, 'L', 0);
		...
		$pdf->setX(5);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell(181, 4, utf8_decode('Valor Físico Total:'), 'TBL', 0, 'R', 0);
		$pdf->SetFillColor(246, 229, 115);
		$pdf->Cell(25, 4, number_format($ff->total_valor_fisico, 2, '.', ','), 'TBLR', 1, 'R', 1);
		...
		$pdf->Rect(5, $yL, 70, 50, false);
		$pdf->Rect(75, $yL, 70, 50, false);
		$pdf->Rect(145, $yL, 66, 50, false);
