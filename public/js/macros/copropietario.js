...
$(document).on('click','.agregarPersonaFisica',function () {
    ...
    var rfc2 = clon.find('input')[1];

    var nombres = clon.find('input')[2];
    var nombres2 = clon.find('input')[2];

    var apellido_paterno = clon.find('input')[3];
    var apellido_paterno2 = clon.find('input')[3];
    ...
    rfc2.id = rfc2.id.replace(/(\w+)\[(\d+)\]\[(\w+)\]/, "$1[" + num + "][$3]");

    nombres.name = nombres.name.replace(/(\w+)\[(\d+)\]\[(\w+)\]/, "$1[" + num + "][$3]");
    nombres2.id = nombres2.id.replace(/(\w+)\[(\d+)\]\[(\w+)\]/, "$1[" + num + "][$3]");

    apellido_paterno.name = apellido_paterno.name.replace(/(\w+)\[(\d+)\]\[(\w+)\]/, "$1[" + num + "][$3]");
    apellido_paterno2.id = apellido_paterno2.id.replace(/(\w+)\[(\d+)\]\[(\w+)\]/, "$1[" + num + "][$3]");
...
$(document).on('click','.quitarPersona',function () {
    ...
    if (confirma == true) {
      ...
      //  alert(len);
        if (len > 1) {
            var divBorrar = ($(this).parent().parent().parent().parent().attr('id'));
            $('#' + divBorrar).remove();
        }
        else {

            $(this).parent().parent().parent().parent().find('input').val(null);
        }
    }
});
