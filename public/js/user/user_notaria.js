 ...
 */
    ...
    {
        var urlsave = decodeURIComponent(laroute.action('admin_usuarioNotariaController@store', { format : 'json' })),
            urlUpdate = decodeURIComponent(laroute.action('admin_usuarioNotariaController@update', {user : ':id', format : 'json' })),
            urlActive = decodeURIComponent(laroute.action('admin_usuarioNotariaController@active', {user : ':id'})),
            urlGetAll = decodeURIComponent(laroute.action('admin_usuarioNotariaController@all'));
                ...
                items.forEach(function(item, i){
                    switch (filter){
                        ...
                        case 'municipio':
                            ...
                            words.forEach(function( word ){
                                var expWord;
                                // Se reivsa que el usuario tenga notarias, si no los tiene no tiene caso hacer la busqueda
                                if( item.notarias.length == 0){
                                    ...
                                    item.notarias.forEach(function (municipio) {
                            ...
                            // Por ultimo se reivsa si el usuario no tiene notarias y se esta buscando "todos notarias"
 ...
 */
    controller('UserCtrl', function($scope, $modal, $timeout, $location, $anchorScroll, Users) {
        ...
        $scope.edit = function(idx, id){
            ...
            // Se marcan los notarias a los que esta asignado el usuario
            $scope.user.notarias.forEach(function(val){
                $scope.notarias.push(val.id_notaria);
            });
