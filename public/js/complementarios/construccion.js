...
angular.module('construccion', ['ngAnimate', 'ngResource', 'ngSanitize','ui.bootstrap']).
    /**
    ...
    */
     ...
     */
    factory('construcciones', function($resource)
    {
        ...
        var urlDestroy = decodeURIComponent(laroute.action('complementarios_ComplementariosController@getEliminarConstruccion', { eliminar : ':gid_construccion' }));
        return $resource(urlsave, {},
            {
                ...
                destroy : {method:'DELETE', params: {gid_construccion : '@gid_construccion' }, isArray: false, url: urlDestroy}
            });
    }).
    /**
     ...
     *
     */
    controller('ConstruccionesCtrl', function($scope, $modal, Permissions) {
         ...
         * @param idx
         */
        var deleteConstruccion= function(idx){
            // Se revisa si el permiso esta guardado en el servidor
            if($scope.construccion[idx].gid_construccion !== undefined){
                Construcciones.destroy({ gid_construccion : $scope.construccion[idx].gid_construccion });
            }
            // Se elimina el permiso del arreglo
            $scope.construccion.splice(idx, 1);
        };
        ...
        $scope.destroy = function (idx) {
            ...
            var modalInstace = $modal.open({
                ...
                resolve    : {
                    permission  : function(){
                        return {
                            clave: $scope.construccion[idx].clave,
                            idx: idx,
                            destroy: function (idx) {
                                deleteConstruccion(idx);
                            }
                        }
                    }
                }
